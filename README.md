...
- **Authentication** — email/password with 6-digit-code or link-based email verification,
  passwordless sign-in via a one-time emailed code, password reset via that same emailed code, and
  "Continue with GitHub" OAuth as a second login option. JWT-based sessions.
